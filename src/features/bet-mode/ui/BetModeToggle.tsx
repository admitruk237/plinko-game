...
export const BetModeToggle = ({ value, onChange, disabled }: Props) => {
  return (
    <div
      className={`relative flex w-full h-[36px] shrink-0 bg-panel-dark rounded-[14px] p-[3px]${disabled ? ' opacity-50 pointer-events-none' : ''}`}
    >
      <div
        className="absolute top-[3px] bottom-[3px] rounded-[11px] bg-neutral-800/30 border border-neutral-800/50 border-t-neutral-800 transition-transform duration-300 ease-in-out"
        style={{
          left: '3px',
          width: 'calc(50% - 3px)',
