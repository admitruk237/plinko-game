      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit((values) => register(values))} className="flex flex-col">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="mt-[16px]">
                ...
                <FormMessage />
                <p className="text-xs text-zinc-500">At least 8 characters with a letter and digit</p>
              </FormItem>
            )}
          />

          {form.formState.errors.root && (
            <p className="text-sm text-red-500 mt-[16px]">
          ...
          <p className="text-center text-[14px] font-normal leading-[20px] tracking-[-0.15px] text-auth-text-muted mt-[25px]">
            ...
            <Link href={ROUTES.LOGIN} className="text-brand-green-start hover:text-brand-green-start/80 font-medium transition-colors">
              Sign In
            </Link>
          </p>
        </form>
